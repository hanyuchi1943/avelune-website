...
export function Footer(){return <footer className="footer"><div><Link className="wordmark" href="/">AVELUNE</Link><p>A quiet place for their love to stay.</p></div><div className="footer-links">{links.map(([name,href])=><Link href={href} key={href}>{name}</Link>)}<a href="https://instagram.com" aria-label="Instagram placeholder">Instagram</a><a href="https://pinterest.com" aria-label="Pinterest placeholder">Pinterest</a><a href="mailto:hello@avelune.com">Email</a></div><div className="footer-legal"><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></div><small>© {new Date().getFullYear()} AVELUNE</small></footer>}
